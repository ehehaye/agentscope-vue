  ...
  >
    <div class="tw-flex tw-max-h-70vh tw-flex-col tw-gap-3 tw-overflow-y-auto">
      ...
      <div>
        ...
        <p class="tw-text-xs tw-text-muted-foreground">主模型失败时的备用模型</p>
      </div>
      <el-select
      ...
      <!-- TTS 模型 -->
      <div>
        <p class="tw-text-sm tw-font-medium tw-text-foreground">TTS 模型</p>
        <p class="tw-text-xs tw-text-muted-foreground">语音合成模型</p>
      </div>
      <el-select
        :value="ttsValueKey"
        placeholder="无"
      ...
      <div v-if="entries.length === 0" class="tw-text-xs tw-text-muted-foreground">
        当前模型无可配置参数
      </div>
